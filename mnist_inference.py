...
# 定义卷积神经网络的前向传播过程。这里ｉ添加了一个新的参数train，用于区分训练过程和测试过程。
# 在这个程序中将用到dropout方法，dropout可以进一步提升模型可靠性并防止过拟合，dropout过程只在训练时使用。
def inference(input_tensor, train, regularizer):
    # 声明第一层卷积层的变量并实现前向传播过程。
    # 通过使用不同的命名空间来隔离不同层的变量，这可以让每一层中的变量命名只需要考虑在当前层　的作用，而不需要担心重命名的问题。
    # 和标准LeNet-5模型不大一样，这里定义的卷积层输入为28×28×1的原始MNIST图片像素。
    ...
    with tf.variable_scope('layer1-conv1'):
        ...
        # 使用边长为5，深度为32的过滤器，过滤器移动的步长为１，且使用全０填充。
        conv1 = tf.nn.conv2d(
    ...
    # 这一层的输入是拉直之后的一组向量，向量长度为3136，输出是一组长度为512的向量。
